...
class Main:
	...
	def inGameUpdate(self):
		...
		if self.respawn_counter >= self.respawn_now and self.lives > 0 and self.delaying_respawn == True:
			...
			list(map(lambda b: b.kill(), self.monster_bullet_group))
			list(map(lambda b: b.kill(), self.player_bullet_group))
			list(map(lambda m: m.kill(), self.monster_group))
			pygame.event.clear()
			self.levelInit()
			self.initGame(self.playerList[2][1])
		...
		for m in self.monster_group:
			...
			if m.giving_charge_points == True:
				...
				self.reward_given = self.monster_hit_reward
